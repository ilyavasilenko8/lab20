...
void deleteByPosSaveOrder_(int arr[], int *n, int pos) {
    if (pos < 0 || pos >= *n) {
        printf("pos in correct\n");
        return;
    }
...
int binarySearch_(int arr[], int n, int x) {
    int left = 0;
    int right = n - 1;

    // Для упорядоченного массива используем бинарный поиск
    ...
    else { // Для неупорядоченного массива используем линейный поиск
        for (int i = 0; i < n; ++i) {
            if (arr[i] == x)
                return i;
        }
