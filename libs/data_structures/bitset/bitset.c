...
bitset bitset_symmetricDifference(bitset set1, bitset set2) {
    bitset result;
    result.maxValue = set1.maxValue > set2.maxValue ? set1.maxValue : set2.maxValue;
    result.values = (set1.values | set2.values) & ~(set1.values &
                                                    set2.values); // используем побитовые операции для нахождения симметрической разности
    return result;
}
